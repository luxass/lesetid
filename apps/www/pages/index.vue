...
<template>
    ...
    <main class="mt-8 flex flex-col gap-8">
        ...
        <textarea v-model="debounced" aria-label="Write your article here!"
        ...
        <section v-if="!error || examples" id="examples" class="mt-8">
            ...
            <p>
                If you want to open a provider different from <NuxtLink href="https://stackblitz.com" target="_blank"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 mt-4 gap-4 lg:grid-cols-3 sm:gap-3">
                <NuxtLink v-for="example in examples" :key="example.handle"
                    ...
                    <ClientOnly>
                        <img v-if="(example.iconUrl && typeof example.iconUrl === 'object') && mode.value === 'dark'"
                            width="32" height="32" :src="example.iconUrl.dark" :alt="`Icon for ${example.title}`">
                        <img v-if="(example.iconUrl && typeof example.iconUrl === 'object') && mode.value !== 'dark'"
                            width="32" height="32" :src="example.iconUrl.light" :alt="`Icon for ${example.title}`">
                        <img v-if="example.iconUrl && typeof example.iconUrl === 'string'" :src="example.iconUrl" width="32"
